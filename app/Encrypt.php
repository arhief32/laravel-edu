...
class Encrypt
{
	...
	{
		//var_dump('ENCODE ::'.$string . ' |' .$key . ' |'.base64_encode(self::mcrypt_encode($string, self::get_key($key))));
	...
	public static function decode($string, $key = '')
	{
		...
		//var_dump('DECODE::'.self::mcrypt_decode(base64_decode($string), self::get_key($key)));
	 ...
	 * @return	string
	 */
	public static function encode_from_legacy($string, $legacy_mode = MCRYPT_MODE_ECB, $key = '')
	{
		...
		self::set_mode($legacy_mode);

		$key = self::get_key($key);
		$dec = base64_decode($string);
		if (($dec = self::mcrypt_decode($dec, $key)) === FALSE)
		{
			self::set_mode($current_mode);
			return FALSE;
		}

		$dec = self::_xor_decode($dec, $key);

		// set the mcrypt mode back to what it should be, typically MCRYPT_MODE_CBC
		self::set_mode($current_mode);

		// and re-encode
		return base64_encode(self::mcrypt_encode($dec, $key));
	}
	 ...
	 * @return	string
	 */
	public static function _remove_cipher_noise($data, $key)
	{
		$key = self::hash($key);
		$str = '';
